...
def parse_route(query: str):
    ...
    # Pattern: X to Y (no "from" keyword, e.g. "india to uk", "Delhi to Tokyo")
    match = re.search(
        r"^(.+?)\s+\bto\s+(.+?)(?:\s+(?:on|for|under|including|with|in|at)\b|[.!?]|$)",
        q_lower,
    )
    ...
    if match:
        ...
        dep_iata = resolve_location_to_iata(origin_text)
        arr_iata = resolve_location_to_iata(dest_text)
        # Only use this match if both sides resolved to a real IATA
    ...
    # Fallback: find country/city mentions (sorted by position in query to preserve order)
    mentions = find_location_mentions(q)
    mentions.sort(key=lambda m: q_lower.find(m))
...
def search_flights(query: str, limit: int = 10):
    ...
    print(f"query: {query}")
    print(f"Test 1 dep_iata: {dep_iata}, arr_iata: {arr_iata}")
